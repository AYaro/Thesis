...
func (r *EntityResultType) GetItems(ctx context.Context, db *gorm.DB, opts GetItemsOptions, out interface{}) error {
	...
	err := r.Query.Apply(ctx, dialect, r.SelectionSet, &wheres, &wws, &joins)
	if err != nil {
		return err
	}
	...
	if r.Filter != nil {
		err = r.Filter.Apply(ctx, dialect, &wheres, &wws, &havings, &hvs, &joins)
		if err != nil {
			return err
		}
	}
...
func (r *EntityResultType) GetCount(ctx context.Context, db *gorm.DB, opts GetItemsOptions, out interface{}) (count int, err error) {
	...
	err = r.Query.Apply(ctx, dialect, r.SelectionSet, &wheres, &wvs, &joins)
	if err != nil {
		return 0, err
	}

	if r.Filter != nil {
		err = r.Filter.Apply(ctx, dialect, &wheres, &wvs, &havings, &hvs, &joins)
		if err != nil {
			return 0, err
		}
	}
...
func (r *EntityResultType) GetAggregations(ctx context.Context, db *gorm.DB, opts GetAggregationsOptions, model interface{}, out interface{}) error {
	...
	if len(wheres) > 0 {
		subq = subq.Where(strings.Join(wheres, " AND "), wvs...)
	}
	if len(havings) > 0 {
		subq = subq.Having(strings.Join(havings, " AND "), hvs...)
	}
